...
//Calculating the hash at each level and returning the resulting hash of Node n.
func (n *MerkleNode) verifyNode() ([]byte, error) {
	if n.leaf {
...
//calculateNodeHash calculates the hash of the node.
func (n *MerkleNode) calculateNodeHash() ([]byte, error) {
	if n.leaf {
		return n.C.CalculateHash()
	}
...
//buildWithContent generates a corresponding tree and returns the root node, a list of leaf nodes, and a possible error.
//Returns an error if cs contains no Contents.
func buildWithContent(cs []Content) (*MerkleNode, []*MerkleNode, error) {
	if len(cs) == 0 {
		return nil, nil, errors.New("error: cannot construct tree with no data")
	}
...
//buildIntermediate construct the intermediate and root levels of the tree. Returns the resulting root node of the tree.
func buildIntermediate(nl []*MerkleNode) (*MerkleNode, error) {
...
//RebuildTree rebuilds the tree reusing only the content that it holds in the leaves.
...
//RebuildTreeWith replaces the content of the tree and does a complete rebuild. Returns an error if the
...
//Returns true if the expected Merkle Root is equivalent to the Merkle root calculated on the critical path for a given content.
func (m *MerkleTree) VerifyData(data Data) (bool, error) {
	for _, l := range m.Leafs {
		ok, err := l.C.Equals(data)
		if err != nil {
			return false, err
		}
